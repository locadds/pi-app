/** Renderer preload allowlist — sync with registerHandler / ipcMain.handle in Main. */
export const IPC_INVOKE_CHANNELS = [
  'ipc:adapter.action.run',
  'ipc:adapter.config.get',
  'ipc:adapter.config.set',
  'ipc:adapter.field.options',
  'ipc:adapter.sidePanel.getState',
  'ipc:adapters.catalog',
  'ipc:adapters.json.catalog',
  'ipc:alerts.signal',
  'ipc:alerts.test',
  'ipc:app.checkUpdate',
  'ipc:app.dismissUpdatePrompt',
  'ipc:app.downloadUpdate',
  'ipc:app.getPendingUpdate',
  'ipc:app.ignoreUpdateVersion',
  'ipc:app.openRelease',
  'ipc:asr.builtinStatus',
  'ipc:asr.detectBinary',
  'ipc:asr.importCodexAccessToken',
  'ipc:asr.probeCodexAuth',
  'ipc:asr.testConnection',
  'ipc:asr.transcribe',
  'ipc:clipboard.writeTempImage',
  'ipc:commands.completions',
  'ipc:commands.list',
  'ipc:context.preview',
  'ipc:dialog:openDirectory',
  'ipc:dialog:openFiles',
  'ipc:extension.cancelUI',
  'ipc:extension.config.get',
  'ipc:extension.config.set',
  'ipc:extension.respondUI',
  'ipc:extensions.list',
  'ipc:extensions.missingRuntimePackages',
  'ipc:extensions.setEnabled',
  'ipc:extensions.setOverride',
  'ipc:extensions.syncGitPackages',
  'ipc:model.cycle',
  'ipc:model.list',
  'ipc:model.set',
  'ipc:pi.getInfo',
  'ipc:pi.models.fetch',
  'ipc:pi.models.get',
  'ipc:pi.models.set',
  'ipc:pi.settings.get',
  'ipc:pi.settings.set',
  'ipc:project.removeRecent',
  'ipc:prompt.abort',
  'ipc:prompt.dequeueClearQueue',
  'ipc:prompt.followUp',
  'ipc:prompt.send',
  'ipc:prompt.steer',
  'ipc:prompts.list',
  'ipc:registry.refresh',
  'ipc:resource.read',
  'ipc:resource.restore',
  'ipc:resource.revision.read',
  'ipc:resource.revisions',
  'ipc:resource.write',
  'ipc:resources.list',
  'ipc:review.commit',
  'ipc:review.getDiff',
  'ipc:review.stageHunks',
  'ipc:review.unstageHunks',
  'ipc:rewind.checkpoints',
  'ipc:rewind.runCommand',
  'ipc:rightPanels.catalog',
  'ipc:rightPanels.saveLayout',
  'ipc:runtime.getState',
  'ipc:sdk.install',
  'ipc:sdk.listAvailable',
  'ipc:sdk.status',
  'ipc:sdk.switch',
  'ipc:session.branchAnchors',
  'ipc:session.clone',
  'ipc:session.compact',
  'ipc:session.delete',
  'ipc:session.export',
  'ipc:session.fork',
  'ipc:session.forkCandidates',
  'ipc:session.getMessages',
  'ipc:session.list',
  'ipc:session.navigateTree',
  'ipc:session.new',
  'ipc:session.open',
  'ipc:session.prepare',
  'ipc:session.reloadFromDisk',
  'ipc:session.rename',
  'ipc:session.setEphemeralDraft',
  'ipc:session.setPendingBind',
  'ipc:session.setVisible',
  'ipc:session.tree',
  'ipc:settings.get',
  'ipc:settings.set',
  'ipc:shell.openPath',
  'ipc:shell.readImagePreview',
  'ipc:shell.showItemInFolder',
  'ipc:skills.applyOverrides',
  'ipc:skills.description.write',
  'ipc:skills.list',
  'ipc:skills.setEnabled',
  'ipc:skills.transfer',
  'ipc:slash.normalize',
  'ipc:slash.resolve',
  'ipc:thinkingLevel.set',
  'ipc:window:close',
  'ipc:window:close-decision',
  'ipc:window:close-decision-shown',
  'ipc:window:isMaximized',
  'ipc:window:maximize',
  'ipc:window:minimize',
  'ipc:wsl.listDistros',
  'ipc:wsl.probeDistro',
  'ipc:workspace.ensureWorker',
  'ipc:workspace.fs.listDir',
  'ipc:workspace.fs.readText',
  'ipc:workspace.fs.rename',
  'ipc:workspace.fs.search',
  'ipc:workspace.isSandbox',
  'ipc:workspace.open',
  'ipc:workspace.sandbox.create',
  'ipc:workspace.sandbox.delete',
  'ipc:workspace.sandbox.list',
  'ipc:workspace.sandbox.rename',
  'ipc:workspace.switch',
  // 小规 Agent 集成（src/main/xiaogui/ipc-handlers.ts 注册；渲染进程经
  // ipcClient.invoke('xiaogui.*') 调用，模式切换 / DESIGN Tool 调用 / sidecar 状态）
  'ipc:xiaogui.guard.status',
  'ipc:xiaogui.delivery.apply.reconcile',
  'ipc:xiaogui.delivery.apply.recovery.prepare',
  'ipc:xiaogui.delivery.apply.retry',
  'ipc:xiaogui.delivery.batch.return',
  'ipc:xiaogui.delivery.gate.approve',
  'ipc:xiaogui.delivery.selection.submit',
  'ipc:xiaogui.coding.context.snapshot',
  'ipc:xiaogui.coding.checkpoint.capture',
  'ipc:xiaogui.coding.checkpoint.list',
  'ipc:xiaogui.coding.checkpoint.restore.preview',
  'ipc:xiaogui.coding.checkpoint.restore.confirm',
  'ipc:xiaogui.coding.direct.checkpoint.list',
  'ipc:xiaogui.coding.direct.checkpoint.restore.preview',
  'ipc:xiaogui.coding.direct.checkpoint.restore.confirm',
  'ipc:xiaogui.coding.roles.attempt.bind',
  'ipc:xiaogui.coding.roles.attempt.read',
  'ipc:xiaogui.coding.roles.copy',
  'ipc:xiaogui.coding.roles.list',
  'ipc:xiaogui.coding.roles.readForEdit',
  'ipc:xiaogui.coding.roles.resetDefault',
  'ipc:xiaogui.coding.roles.upsert',
  'ipc:xiaogui.coding.plan.observe',
  'ipc:xiaogui.coding.plan.perform',
  'ipc:xiaogui.coding.review.read',
  'ipc:xiaogui.hub.observe',
  'ipc:xiaogui.hub.execution.start',
  'ipc:xiaogui.hub.execution.startBatch',
  'ipc:xiaogui.hub.perform',
  'ipc:xiaogui.hub.read',
  'ipc:xiaogui.hub.readEvents',
  'ipc:xiaogui.hubTask.status',
  'ipc:xiaogui.hubTask.connect',
  'ipc:xiaogui.c2.status',
  'ipc:xiaogui.c2.confirm',
  'ipc:xiaogui.c2.cancel',
  'ipc:xiaogui.c2.app.open',
  'ipc:xiaogui.kimi.login.start',
  'ipc:xiaogui.kimi.status',
  'ipc:xiaogui.mode.get',
  'ipc:xiaogui.mode.switch',
  'ipc:xiaogui.officeSurface.mode.get',
  'ipc:xiaogui.officeSurface.gateway.snapshot.read',
  'ipc:xiaogui.officeSurface.gateway.snapshot.write',
  'ipc:xiaogui.officeSurface.session.prepare',
  'ipc:xiaogui.officeSurface.session.release',
  'ipc:xiaogui.phase.get',
  'ipc:xiaogui.phase.switch',
  'ipc:xiaogui.scope.baselineProjects',
  'ipc:xiaogui.scope.get',
  'ipc:xiaogui.scope.list',
  'ipc:xiaogui.scope.lookup',
  'ipc:xiaogui.scope.set',
  'ipc:xiaogui.sidecar.status',
  'ipc:xiaogui.tool.invoke',
  'ipc:xiaogui.templateLibrary.configuration.get',
  'ipc:xiaogui.templateLibrary.configuration.choose',
  'ipc:xiaogui.templateLibrary.list',
  'ipc:xiaogui.templateLibrary.detail',
  'ipc:xiaogui.templateLibrary.usage',
  'ipc:xiaogui.templateLibrary.trash',
  'ipc:xiaogui.templateLibrary.restore',
  'ipc:xiaogui.templateLibrary.purge',
  'ipc:xiaogui.templateLibrary.preview.prepare',
  'ipc:xiaogui.templateLibrary.preview.release',
  'ipc:xiaogui.templateReview.document.read',
  'ipc:xiaogui.templateReview.image.choose',
  'ipc:xiaogui.work.template-intake.source.choose',
  'ipc:xiaogui.work.template-intake.review.open',
  'ipc:xiaogui.work.docx.confirm',
  'ipc:xiaogui.work.docx.cancel',
  'ipc:xiaogui.work.docx.discover',
  'ipc:xiaogui.work.docx.output.access',
  'ipc:xiaogui.work.docx.prepare',
] as const

export type IpcInvokeChannel = (typeof IPC_INVOKE_CHANNELS)[number]

const channelSet = new Set<string>(IPC_INVOKE_CHANNELS)

export function isAllowedIpcChannel(channel: string): channel is IpcInvokeChannel {
  return channelSet.has(channel)
}
