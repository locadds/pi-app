...
import type {
  HubTaskExecutionBindingV1,
  HubTaskExecutionTerminalOutcomeV1,
} from '../hub-task/worker-service'
...
export interface HubTaskExecutionLifecycleReconcilerV1 {
  /**
   * One serial, idempotent main-process entry for immediate starts, runtime
   * terminals, verification terminals, interruptions, cancellations, and
   * Delivery creation. It never throws into the local execution path.
   */
  reconcile(trigger: HubTaskExecutionLifecycleTriggerV1): Promise<void>
  /** Startup repair: start evidence first, then local recovery and reconciliation. */
  recover(): Promise<void>
}

export interface CreateHubTaskExecutionLifecycleCoordinatorOptionsV1 {
...
export function createHubTaskExecutionLifecycleCoordinatorV1(
  options: CreateHubTaskExecutionLifecycleCoordinatorOptionsV1,
...
class HubTaskExecutionLifecycleCoordinatorImpl implements HubTaskExecutionLifecycleReconcilerV1 {
  ...
  constructor(private readonly options: CreateHubTaskExecutionLifecycleCoordinatorOptionsV1) {}

  reconcile(trigger: HubTaskExecutionLifecycleTriggerV1): Promise<void> {
    if (this.recovering && this.recovery) {
      return this.recovery.then(() => this.enqueue(trigger)).catch(() => undefined)
    }
  ...
  private async recoverOnce(): Promise<void> {
    ...
    try {
      // Hub start evidence must be durable before any recovered terminal result
      // is considered. A binding with no local execution authority is ignored.
      ...
      for (const binding of [...initial, ...this.bindings()]) {
        targets.set(triggerKey(binding), binding)
      }
  ...
  private enqueue(trigger: HubTaskExecutionLifecycleTriggerV1): Promise<void> {
    const run = this.tail
      .then(() => this.reconcileOnce(trigger))
      .catch(() => undefined)
    this.tail = run
    return run
  }
  ...
  private async reconcileOnce(trigger: HubTaskExecutionLifecycleTriggerV1): Promise<void> {
    ...
    const delivery = this.options.delivery.readLatestDelivery(trigger.address, trigger.flowId)
    ...
    if (verificationState) {
      await this.options.evidence.reportExecutionOutcome(
        trigger.address,
        trigger.flowId,
        { verificationState },
      )
  ...
  private async resolveAuthority(
    trigger: HubTaskExecutionLifecycleTriggerV1,
  ): Promise<{ taskRun: TaskRunProjectionM2BV1; attempt: AttemptProjectionM2BV1 } | null> {
    try {
      ...
      const taskRun = selectTaskRun(projection, trigger.taskRunId)
      if (!taskRun) return null
      const attempt = selectAttempt(projection, taskRun, trigger.attemptId)
      return attempt ? { taskRun, attempt } : null
    } catch {
      return null
    }
  }
  ...
  private bindings(): readonly HubTaskExecutionBindingV1[] {
    try {
      return this.options.evidence.listExecutionBindings()
    } catch {
      return []
    }
  }
  ...
  private async safe(operation: () => Promise<void>): Promise<void> {
    try {
      ...
      // Persisted local authority and evidence remain retryable on next startup.
    }
  }
}
...
): TaskRunProjectionM2BV1 | null {
  if (taskRunId) {
    return projection.taskRuns.find((candidate) => candidate.taskRunId === taskRunId) ?? null
  }
  return projection.taskRuns.length === 1 ? projection.taskRuns[0]! : null
}
...
): AttemptProjectionM2BV1 | null {
  if (attemptId && taskRun.attemptId && taskRun.attemptId !== attemptId) return null
  const expectedAttemptId = attemptId ?? taskRun.attemptId
  if (expectedAttemptId) {
    const attempt = projection.attempts.find((candidate) => candidate.attemptId === expectedAttemptId)
    return attempt?.taskRunId === taskRun.taskRunId ? attempt : null
  }
  const attempts = projection.attempts.filter((candidate) => candidate.taskRunId === taskRun.taskRunId)
  return attempts.length === 1 ? attempts[0]! : null
}

function executionTerminalState(
  authority: { taskRun: TaskRunProjectionM2BV1; attempt: AttemptProjectionM2BV1 },
): HubTaskExecutionTerminalOutcomeV1['verificationState'] | null {
  const { taskRun, attempt } = authority
  if (
    taskRun.status === 'OUTCOME_UNKNOWN'
    || attempt.status === 'OUTCOME_UNKNOWN'
    || attempt.verificationSummary?.state === 'OUTCOME_UNKNOWN'
  ) return 'UNKNOWN'
  ...
  return attempt.verificationSummary?.state === 'FAILED' ? 'FAIL' : 'NOT_RUN'
}
...
function triggerKey(trigger: HubTaskExecutionLifecycleTriggerV1): string {
  return [
    trigger.address.projectId,
    trigger.address.sessionKey,
    trigger.flowId,
    trigger.taskRunId ?? '',
    trigger.attemptId ?? '',
  ].join('\u0000')
