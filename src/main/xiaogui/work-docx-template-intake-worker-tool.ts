...
function messageForError(code: TemplateIntakeErrorCodeV1): string {
  switch (code) {
    ...
    case 'TEMPLATE_INTAKE_SCOPE_MISMATCH':
      return '当前会话尚未准备好 Word 整理能力，请重新进入会话后再试'
    case 'TEMPLATE_INTAKE_MODE_NOT_ALLOWED':
      return '普通成品 Word 整理只在日常工作会话中可用'
    case 'TEMPLATE_INTAKE_INPUT_INVALID':
      return '整理操作参数或所选 Word 不符合要求，请检查后重试'
    case 'TEMPLATE_INTAKE_INPUT_TOO_LARGE':
      return 'Word、提取内容或整理报告超过安全上限，未进行静默截断'
    case 'TEMPLATE_INTAKE_UNSAFE_DOCX':
      return '所选 Word 未通过本地安全检查，请更换文档'
    case 'TEMPLATE_INTAKE_OPERATION_ACTIVE':
      return '当前会话已有一份 Word 正在分析，请等待完成或先取消'
    ...
    case 'TEMPLATE_INTAKE_SOURCE_MISSING':
      return '源 Word 已丢失，且没有重新选择摘要相同的文件'
    case 'TEMPLATE_INTAKE_SOURCE_CHANGED':
      return '源 Word 已变化，原整理报告已失效，必须重新分析'
    case 'TEMPLATE_INTAKE_PARSER_FAILED':
      return 'Word 解析失败或超时，没有创建不完整报告'
...
export function createXiaoguiWorkDocxTemplateIntakeWorkerToolHandlerV1(
  ...
  return async ({ request, fromCwd, sessionFile, fromSessionId, signal }) => {
    ...
    if (!parsed.success) {
      return failure('HOST_TOOL_REQUEST_INVALID', 'Word 整理参数不完整，请重新表达需求后再试')
    }
    ...
    if (parsed.data.payload.sourceSessionId !== fromSessionId) {
      return failure('SESSION_SCOPE_MISMATCH', '当前会话已经切换，请重新发起 Word 整理')
    }
    if (signal?.aborted) return failure('HOST_TOOL_ABORTED', 'Word 整理已取消')
    ...
    if (scope.sessionMode !== 'WORK') {
      return failure('TEMPLATE_INTAKE_MODE_NOT_ALLOWED', '普通成品 Word 整理只在日常工作会话中可用')
    }
    ...
    if (inFlightScopes.has(key)) {
      return failure('TEMPLATE_INTAKE_OPERATION_ACTIVE', '当前会话正在处理另一项 Word 整理操作')
    }
    ...
    try {
      ...
      return failure('HOST_TOOL_FAILED', 'Word 整理失败，请稍后重试')
    } finally {
