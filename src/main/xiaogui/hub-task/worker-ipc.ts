...
function mapItemResult(
  result: Awaited<ReturnType<HubTaskWorkerServiceV1['openAssignment']>>,
) {
  return result.ok
    ? { ok: true as const, value: toPublicInboxItem(result.value) }
    : result
}
