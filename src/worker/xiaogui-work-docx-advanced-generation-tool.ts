...
function publicText(details: SafeDetails): string {
  switch (details.kind) {
    ...
    case 'XIAOGUI_WORK_DOCX_ADVANCED_GENERATION_PREPARED': return `已生成并打开成品文档只读预览：重复记录 ${details.plan.repeatRecordCount} 条、保留条件块 ${details.plan.retainedConditionalCount} 项。原模板未修改。如确认继续，请单独回复“确认”。`
    case 'XIAOGUI_WORK_DOCX_ADVANCED_GENERATION_RESUMED': return details.receipt ? '已恢复已发布的成品文档记录。' : details.plan ? '已重新核对模板并打开成品文档预览。原模板未修改。' : '已恢复模板结构摘要，可以继续补齐成品文档数据。'
    case 'XIAOGUI_WORK_DOCX_ADVANCED_GENERATION_TARGET_SELECTION_CANCELLED': return '已取消选择成品文档保存位置；预览仍保留，可稍后再次确认。'
    case 'XIAOGUI_WORK_DOCX_ADVANCED_GENERATION_PUBLISHED': return `成品文档已另存完成：重复记录 ${details.receipt.repeatRecordCount} 条、保留条件块 ${details.receipt.retainedConditionalCount} 项。原模板未修改。`
    case 'XIAOGUI_WORK_DOCX_ADVANCED_GENERATION_CANCELLED': return '已取消高级 Word 生成并清理受控预览；原模板未修改。'
    case 'XIAOGUI_WORK_DOCX_ADVANCED_GENERATION_ACCESSED': return details.action === 'OPEN' ? '已打开成品文档。' : '已在文件夹中定位成品文档。'
