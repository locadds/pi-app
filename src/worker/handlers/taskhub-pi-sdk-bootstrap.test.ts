...
it.each(['WORK', 'DESIGN', 'CODING'] as const)('boots the existing %s Pi harness with only the Attempt tools', async mode => {
  ...
  const expected = mode === 'CODING' ? ['edit', 'read', 'write']
  ...
  if (mode === 'DESIGN') {
    ...
    expect(Value.Check(parameters, { action: 'open' })).toBe(true)
    expect(Value.Check(parameters, { action: 'inspect' })).toBe(true)
    expect(Value.Check(parameters, {})).toBe(true)
    ...
    const execution = definition.execute(
      'design-call-1', { action: 'open' } as never, undefined, undefined, undefined as never,
    )
