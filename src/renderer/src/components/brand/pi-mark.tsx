...
/** 小规 A 方案控制台小标。保留 PiMark 导出名以兼容既有引用。 */
function PiMarkImpl({
  className,
  size = 16,
  inverted = false,
  ...
  const maskId = `xiaogui-console-cut-${useId().replaceAll(':', '')}`
  const compact = size < 20
  const foreground = inverted ? '#F7F5EF' : 'currentColor'
  const node = compact ? foreground : '#E8463A'

  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="200 0 220 220"
      width={size}
      height={size}
      className={cn('shrink-0', className)}
      aria-hidden
    >
      <mask id={maskId} x="0" y="0" width="755" height="220" maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse">
        <rect width="755" height="220" fill="#fff" />
        <path
      ...
      <circle cx="344" cy="87" r="7" fill={node} />
      <path d="M344 74v26M331 87h26" stroke={foreground} strokeWidth="1.8" />
      <circle cx="344" cy="87" r="5.6" fill={node} />
    </svg>
  )
}
