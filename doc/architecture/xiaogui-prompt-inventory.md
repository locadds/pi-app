...
只有 `DEFAULT` 自动激活。WORK 的资料读取、标准 Word、模板整理和模板生成均为默认候选，Host 默认清单从同一矩阵派生；它们的可见性不依赖用户是否说中关键词，也不依赖 sticky 续接。Pi 根据 Skill 与工具说明理解任务，实际工具仍受阶段、已注册 Runtime 和既有授权门控制。ASK/PLAN 仍只提供只读工具，EXECUTE 允许选择文档工具，但工具可见不等于批准具体操作。`ALLOWED`、协作和其他模式仍沿用结构化 Context/本地选择规则；跨模式隐藏策略不变。能力缺少所需 Runtime Tool 时不进入 Manifest，也不得声称已执行。
...
该入口保留原有表达及私有文件交接。选择器 `1.2.0` 不再依赖这段消息才能向 WORK 暴露文档工具；长 `.doc`/`.docx` 和自然语言改述使用相同默认工具集。既有本地规则只保留兼容意图元数据及非默认能力选择，不扩大正则跨度。Renderer 只取得显示文件名，绝对路径仍留在 Main 私有交接中。

2026-09-07 稳定性候选新增工具共享规则：普通会话延续（如单独“继续”）不算正式复核、生成或保存批准；生成流程仍等待明确确认或工具指定的确认按钮。该规则不增加聊天授权解析器，不改变 Host 的跨 Run、复核状态和私有令牌验证。验证与剩余验收见 `doc/WORK-CAPABILITY-EVIDENCE-2026-09-07.md`。
...
- 标准报告与成品文档 PREPARE 成功后引导“如确认继续，请单独回复‘确认’”；模板整理 START 成功后引导单独回复“复核”或“打开复核卡”。正式模板物化必须使用小规内置预览按钮取得主进程私有确认令牌；聊天确认本身不授权物化。单独“继续”只表示会话延续，不批准正式操作。
