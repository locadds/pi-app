...
## C2 无会话账号连接整改（2026-09-07）

- 协作面板在无 canonical scope 时显示既有 Hub 登录配对表单；账号连接不再要求创建会话。
- 无 scope 不读取收件箱、不展示任务操作；生成计划函数也要求真实 address。已有会话路径保留。
- 配对仍由表单提交显式调用既有 connect；挂载和会话切换只读本机状态，不自动重新配对。主进程加密、账号身份、节点撤销与全局 C2 安装逻辑均未修改。
- 聚焦 UI 验证：2 文件 / 34 用例通过。类型与构建结果见 `doc/C2-LOGIN-REMEDIATION-HANDOFF.md`。
- 已审产品 SHA：`2aa7d453d25dbd775aa44c6974c4a7443d0351d4`。验收协调任务2026-09-07确认 Standards 0、Spec 0，独立复跑新增2项测试通过。本次登记仅改文档，产品 SHA 不变。
- 下一步：按已审构建准备乙机更新包；未启动两机重验、未合并或发布。
