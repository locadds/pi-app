...
状态：已获独立只读验收 `APPROVE`。允许进入 H1-4 的单机双实例送达验收，但未合并正式主线、未发布，也未授权 H1-4C 结果回传。
...
1. 在新的独立验收工作包中，使用两套隔离小规数据目录与 Hub `770c0a7` 完成“发布—领取/定向—打开—Hub ACK—发布者看到已送达”的单机双实例旅程；此记录不自动启动该工作包。
2. 单机双实例验收通过前不合并、不发布、不进入 H1-4C。
3. 仅在上述送达链路验证后再讨论 H1-4C 的结果回传；继续保持本机计划、执行和最终应用的人工批准门。

## 验收门

本文件记录的是 H1-4B 桌面代码已通过独立审查的状态，不构成正式主线合并、发布、真实网络联调或 H1-4C 的授权。后续单机双实例旅程仍须在独立工作包中产生自己的实际证据与验收记录。
