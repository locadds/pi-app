...
状态：阶段 A 根据第三轮独立审查的 REQUEST CHANGES 完成机器合同、夹具和验证器修订，等待重新独立审查与人工验收；本阶段仅写入审计、合同、夹具和聚焦验证器，没有 H1-4C/C3 产品代码、迁移、路由或前端实现。未合并正式主线、未发布。
...
1. 复跑机器合同/负例与差异检查、提交并推送本次第三轮合同修订；再请独立审查复核规范化字节规则、30 秒超时、可恢复死信 DTO，以及正确鉴权下的错误路由请求体拒绝。
