...
状态：H1-3 已于 2026-09-02 人工验收通过；未合并正式主线。H1-4 将在新的独立分支中施工。
...
- 对接 Hub 分支：`agent/taskhub-h1-worker-api-v1@8fc951d01c8756a2c4f899d962f72bd91b5da03f`；其 H1-3 API 已完成独立复审和推送，仍与本桌面分支一同等待人工验收。
...
- Hub 任务收件箱的真实网络联调保留给 H1-4 的回执/结果闭环；H1-3 两端代码审查与推送已完成，尚未取得人工验收。
...
| 独立代码审查 | 初审发现 2 个高优先级和 1 个中优先级桌面保护缺口，均由 `3390af6` 修复；复审为通过，另列 3 个低优先级收口项，均已由 `cd7f851` 完成（其中 Renderer 吊销清屏新增了先失败、后通过的组件测试）；最终小补丁复核亦为通过。Hub API 端复审同样通过。 |
...
- [P2，带入 H1-4] `http-task-worker-port.ts` 当前把 `409 CONFLICT` 与旧节点 `403 FORBIDDEN` 一并映射为 `NODE_REVOKED`。良性任务状态冲突在边缘重试时可能被误导为“需重新配对”。H1-4 首先拆分该语义：`403` 才清凭据，`409` 保持凭据并提示刷新；同时补聚焦测试。
- [P3，已更正交接记录] 桌面端独立复审证据实际文件为 `taskhub-h1-3-desktop-worker-code-review.md`；早期交接记录中的 `low-close-review` 名称不正确，已在长期交接说明中修正。

## 人工验收记录

- 2026-09-02：独立只读验收结论为 `APPROVE`，无阻断问题；本阶段 10 个桌面聚焦测试文件 / 58 条用例、类型检查、构建、差异检查及 Electron 可见冒烟均已复跑通过。
- Hub Worker API 同轮验收为 38/38 聚焦测试、shared/server typecheck、server build、差异检查及 OpenAPI 5/5 Worker `403 FORBIDDEN` 声明通过。
- 验收未授权直接合并正式主线或发布，也未将 H1-3 表述为跨机送达闭环完成。

## 下一阶段计划

在独立 `TASKHUB-H1-4` 分支中实现签名回执上传/ACK、离线补传、结果摘要和双实例/两机 LAN 受控旅程；继续保持人工批准、工作树和最终应用门不变。H1-4 开工时先处理上述 P2 的 `409`/`403` 语义拆分。

## 验收门

本文件是 H1-3 已验收状态记录。验收通过不等于正式主线合并、发布或跨机送达闭环完成；H1-4 仍须在独立分支中完成并再次验收。
