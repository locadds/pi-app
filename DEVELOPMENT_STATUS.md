...
状态：H1-4A 已于 2026-09-02 独立验收通过；未合并正式主线。H1-4B 将在新的独立分支中施工。
...
- 2026-09-02：独立只读验收结论为 `APPROVE`，无阻断问题。验收方复跑 3 个聚焦测试文件 / 17 条用例、web/node typecheck、build 和 `git diff --check 5e9fc18..HEAD` 均通过。
- 验收确认 H1-3 P2 与同源 P3 已闭环：`409` 与 `403` 语义拆分，并有 HTTP Adapter、Worker 服务和 Renderer 三层回归。
- 验收未授权合并正式主线、发布或提前实现 H1-4B 之外的能力。

## 下一阶段计划

在新的独立桌面 H1-4B 分支与 Hub H1-4 分支上接入签名回执上传、ACK 与离线补传；继续维持“Hub 只识别小规节点、桌面本机选择 Agent、人工批准后才执行/应用”的冻结边界。

## 验收门

本文件是 H1-4A 已验收状态记录，不构成 H1-4 完成或发布授权。H1-4B/H1-4C 仍须各自有实际测试证据和独立验收。
